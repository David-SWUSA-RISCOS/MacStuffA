...
This directory is dedicated to my own attempt at a decently fast BASIC interpreter and compiler.  The syntax is largely based on that of BBC BASIC, with some influence from Atari BASIC.  It should be noted that I am only familiar with two versions of BBC BASIC, one is the version for the Commodore 64/128 the other is BBC BASIC V (also known as ARM BASIC or simply BASIC V).
